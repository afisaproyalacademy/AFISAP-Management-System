...
function viewStudentPhoto(studentId){
  ...
  modal('Student Passport Photo',`
    <div class='student-photo-modal'>
      <img src='${afisapStaffPhotoSrc(s)}' alt='${esc((s.name || [s.firstName,s.middleName,s.surname].filter(Boolean).join(' ')))}' onerror='this.onerror=null;this.src="${afisapStaffPhotoFallback(s).replace(/"/g,"&quot;")}">
...
function afisapStaffPhotoSrc(staff){
  const rawId=String(staff?.passportPhotoFileId||"").trim();
  const rawUrl=String(staff?.passportPhotoUrl||staff?.photo||"").trim();
  const id=afisapDriveFileId(rawId||rawUrl);
  if(id) return afisapDriveFileUrl(id);
  return /^https?:\/\//i.test(rawUrl) ? rawUrl : "";
}
function afisapStaffPhotoFallback(staff){
  const rawId=String(staff?.passportPhotoFileId||"").trim();
  const rawUrl=String(staff?.passportPhotoUrl||staff?.photo||"").trim();
  const id=afisapDriveFileId(rawId||rawUrl);
  return id ? "https://drive.google.com/uc?export=view&id="+encodeURIComponent(id) : rawUrl;
}
                ...
                <tr>
                  <td>
                    ${afisapStaffPhotoSrc(s)
                      ? `<img class='staff-thumb' src='${afisapStaffPhotoSrc(s).replace(/'/g,"&#39;")}' alt='${esc(nm)}' data-staff-photo-id='${esc(s.id)}' onclick='viewStaffPhoto("${s.id}")' title='View passport photo' onerror='this.onerror=null;this.src="${afisapStaffPhotoFallback(s).replace(/"/g,"&quot;")}">`
